...
int cmd_run(const std::vector<std::string>& args) {
    ...
    // Cloud-Device Fusion: parse config and initialize controller.
    auto fusion_cfg = cloud_fusion::parseCloudFusionConfig(config_path.string());
    cloud_fusion::FusionController fusion(fusion_cfg);
    if (fusion.isActive()) {
        std::cout << "  cloud fusion: ENABLED → " << fusion_cfg.model
                  << " (threshold=" << static_cast<int>(fusion_cfg.complexity_threshold * 100)
                  << "%)\n";
    } else if (fusion_cfg.enabled) {
        std::cout << "  cloud fusion: enabled but NOT ACTIVE (missing API key or endpoint)\n";
        std::cout << "    set $" << fusion_cfg.api_key_env << " and configure cloud_fusion.endpoint in agent.yaml\n";
    } else {
        std::cout << "  cloud fusion: disabled (add cloud_fusion.enabled: true in agent.yaml)\n";
    }
    ...
    while (true) {
        ...
        // /cloud [on|off|status|threshold <val>] — runtime cloud fusion control
        if (line == "/cloud" || line.substr(0, 7) == "/cloud ") {
            std::string subcmd = line.size() > 7 ? line.substr(7) : "status";
            if (subcmd == "on") {
                fusion.setEnabled(true);
                std::cout << "  ✓ cloud fusion enabled\n\n";
            } else if (subcmd == "off") {
                fusion.setEnabled(false);
                std::cout << "  ✓ cloud fusion disabled\n\n";
            } else if (subcmd.substr(0, 10) == "threshold ") {
                float t = std::stof(subcmd.substr(10));
                fusion.setThreshold(t);
                std::cout << "  ✓ complexity threshold set to "
                          << static_cast<int>(t * 100) << "%\n\n";
            } else {
                std::cout << "  cloud fusion: "
                          << (fusion.isEnabled() ? "ENABLED" : "DISABLED")
                          << "  active=" << (fusion.isActive() ? "yes" : "no")
                          << "  threshold="
                          << static_cast<int>(fusion.getThreshold() * 100) << "%\n";
                std::cout << "    model: " << fusion.config().model << "\n";
                std::cout << "    endpoint: " << fusion.config().endpoint << "\n";
                std::cout << "    commands: /cloud on | /cloud off | /cloud threshold 0.5\n\n";
            }
            continue;
        }
        }
        ...
        if (!matched && real && runtime) {
            // Cloud-Device Fusion: check if this task should route to cloud.
            auto routing = fusion.route(line);
            bool cloud_handled = false;

            if (routing.route == cloud_fusion::InferenceRoute::Cloud && fusion.isActive()) {
                std::cout << "  ☁ route=cloud  complexity="
                          << static_cast<int>(routing.complexity_score * 100)
                          << "%  model=" << fusion_cfg.model << "\n";
                std::cout << "  " << std::flush;

                auto cloud_result = fusion.cloudInferStream(line,
                    [](const std::string& delta, bool final_chunk) -> bool {
                        std::cout << delta << std::flush;
                        if (final_chunk) std::cout << "\n";
                        return true;
                    });

                if (cloud_result.success) {
                    auto elapsed_c = std::chrono::steady_clock::now() - start;
                    auto ms_c = std::chrono::duration_cast<
                        std::chrono::milliseconds>(elapsed_c).count();
                    std::cout << "  ✓ route=cloud  latency=" << ms_c << "ms"
                              << "  tokens=" << cloud_result.tokens_used
                              << "  (" << routing.reason << ")\n";
                    last_input = line;
                    last_output = cloud_result.content;
                    cloud_handled = true;
                } else if (fusion_cfg.fallback_to_local) {
                    std::cout << "  ! cloud failed (" << cloud_result.error
                              << "), falling back to local\n";
                } else {
                    std::cerr << "  ✗ cloud inference failed: "
                              << cloud_result.error << "\n";
                    cloud_handled = true;
                }
            }
            ...
            }  // end if (!cloud_handled)
        } else if (!matched && fusion.isActive()) {
            // No local model, but cloud fusion is active — route to cloud.
            auto routing = fusion.route(line);
            if (routing.route == cloud_fusion::InferenceRoute::Cloud) {
                std::cout << "  \xe2\x98\x81 route=cloud  complexity="
                          << static_cast<int>(routing.complexity_score * 100)
                          << "%  model=" << fusion_cfg.model << "\n";
                std::cout << "  " << std::flush;
                auto cloud_result = fusion.cloudInferStream(line,
                ...
                if (cloud_result.success) {
                    ...
                    auto ms = std::chrono::duration_cast<
                        std::chrono::milliseconds>(elapsed).count();
                    std::cout << "  \xe2\x9c\x93 route=cloud  latency=" << ms << "ms"
                              << "  (" << routing.reason << ")\n";
                    last_input = line;
                    last_output = cloud_result.content;
                } else {
                    std::cerr << "  \xe2\x9c\x97 cloud inference failed: "
                              << cloud_result.error << "\n";
                }
            } else {
                std::cout << "  \xc2\xb7 complexity="
                          << static_cast<int>(routing.complexity_score * 100)
                          << "% < threshold — no local model configured\n";
                std::cout << "  (download a model with "
                             "`sparx pull qwen2.5-0.5b-instruct`)\n";
            }
        } else if (!matched) {
